...
use user32::{CreateWindowExW, DefWindowProcW, RegisterClassW, GetDlgItem, GetWindowTextLengthW, GetWindowTextW,
	GetMessageW, TranslateMessage, DispatchMessageW, MessageBoxW, PostQuitMessage, SetWindowLongPtrW, GetWindowLongPtrW
};
...
type WinProc = fn(HWND, WPARAM)->i32;
fn def_proc(handle: HWND, wparam: WPARAM) -> i32 {0}
...
impl Win{
	...
	fn procedures(&mut self, handle: HWND, wparam: WPARAM) -> i32 {
		let fun: WinProc = self.wproc;
		fun(handle, wparam)
	}
	
	fn set_procedures(&mut self, wproc: WinProc) -> &mut Win {
		self.wproc = wproc;
		self
	}
...
fn main() {
	...
	fn wproc(handle: HWND, wparam: WPARAM) -> i32{
		unsafe {
			match wparam as c_int{
				BTN => {
					...
					MessageBoxW(handle, edit_str, to_wstring("提示"), MB_OK);
				}
				 _ => {}
			}
		}
